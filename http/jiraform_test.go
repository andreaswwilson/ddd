...
func TestGetJiraForm(t *testing.T) {
	...
	tests := []struct {
		description  string
		expected     *ddd.NewSubscriptionOrder
		jsonResponse string
	}{
		{
		...
		},
		{
			...
			expected:     &ddd.NewSubscriptionOrder{BudgetContact: []string{"lukas.nilsen@skatteetaten.no", "andreaswago.wilson@skatteetaten.no"}},
		},
		{
			...
			expected:     &ddd.NewSubscriptionOrder{VNetSize: 20},
		},
	}
